...
const migrationVersionsQuery = "SELECT COALESCE(group_concat(version), '') FROM (SELECT version FROM feedback_schema_version ORDER BY version)"
...
func TestMigrationMalformedVersionTable(t *testing.T) {
	for _, tc := range []struct{ name, ddl string }{
		{"missing columns", "CREATE TABLE feedback_schema_version (version TEXT); INSERT INTO feedback_schema_version VALUES ('broken')"},
		// Every column the claim needs exists, so only the version read
		// rejects the non-integer row.
		{"non-integer version", "CREATE TABLE feedback_schema_version (version PRIMARY KEY, description TEXT NOT NULL, applied_at INTEGER NOT NULL); INSERT INTO feedback_schema_version VALUES ('broken', 'corrupt', 0)"},
	} {
		t.Run(tc.name, func(t *testing.T) {
			...
			assertMigrationSQL(t, db, "SELECT group_concat(version) FROM feedback_schema_version", "broken")
			assertMigrationSQL(t, db, "SELECT COUNT(*) FROM sqlite_schema WHERE name <> 'feedback_schema_version' AND name NOT LIKE 'sqlite_%'", "0")
		})
	}
}
