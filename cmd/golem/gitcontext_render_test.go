...
// Capture has no frame; only the combined renderer creates keyed boundaries.
func gitBlockBody(t *testing.T, block string) string { t.Helper(); return block }
func TestGitContextBlockRendersLabeledSnapshot(t *testing.T) {
	...
	want :=
		"prefix: sub/dir/ (workspace root; file tools can only access paths under this prefix; strip it for file-tool paths)\n" +
			...
			" M cmd/golem/main.go\n" +
			"?? copilot.json\n"
	got, payload := gitContextBody(st, gitContextMaxBytes, "")
	if got != want {
		t.Fatalf("block:\n got=%q\nwant=%q", got, want)
	}
	...
	want =
		"branch: No commits yet on main\n" +
			...
			"working tree: clean\n"
	if got, _ := gitContextBody(st, gitContextMaxBytes, ""); got != want {
...
func TestGitContextBlockOmitsAbsoluteToplevel(t *testing.T) {
	...
	got, _ := gitContextBody(st, gitContextMaxBytes, "")
...
func TestGitContextBlockNeutralizesBothSentinels(t *testing.T) {
	...
	if n := strings.Count(lower, "<<<git_context"); n != 0 {
...
func TestGitContextBlockEnforcesComponentCap(t *testing.T) {
	...
	if _, bigger := gitContextBody(st, 4*gitContextMaxBytes, ""); bigger <= payload {
...
func TestGitContextBlockBackstopHoldsUnderImpossibleBudget(t *testing.T) {
	...
	for _, budget := range []int{0, 1, 8, 40} {
		got, payload := gitContextBody(st, budget, "")
