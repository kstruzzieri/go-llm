...
func TestZeroWidthTagsTrustedAndBlocksForeign(t *testing.T) {
	cases := []struct {
		...
		{"combining grapheme joiner", agent.OriginForeign, "a\u034fb",
			msgFinding("zero_width", agent.VerdictBlock, 20, agent.OriginForeign, "1 zero-width code point(s), first U+034F")},
		{"soft hyphen", agent.OriginForeign, "a\u00adb",
			msgFinding("zero_width", agent.VerdictBlock, 20, agent.OriginForeign, "1 zero-width code point(s), first U+00AD")},
		{"hair space", agent.OriginForeign, "a\u200ab",
			msgFinding("zero_width", agent.VerdictBlock, 20, agent.OriginForeign, "1 zero-width code point(s), first U+200A")},
		...
		{"escaped selected ignorable", agent.OriginWorkspace, `{"p":"a\u034fb"}`,
			msgFinding("zero_width", agent.VerdictTag, 20, agent.OriginWorkspace, "1 zero-width code point(s), first U+034F (escaped)")},
		{"escapes that are not zero-width are clean", agent.OriginForeign, `{"p":"\u0041\u00e9\uZZZZ\u"}`, nil},
		{"variation selector stays clean", agent.OriginForeign, "\ufe0f", nil},
		{"emoji presentation stays clean", agent.OriginForeign, "❤️", nil},
