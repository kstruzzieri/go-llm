...
Pinned scoped dispatch is supported on Linux and macOS; other platforms advertise only legacy string tasks and reject scoped tasks before starting children. The shared readers also pin directory traversal, reject symlink replacement and special-file reads, and preserve search-only directory access.

When a directory cannot be enumerated, readers verify its opened component names before applying policy, preventing case aliases from bypassing guards. Linux uses procfs and macOS uses descriptor path metadata for this check; access fails closed if canonical spelling cannot be verified. In particular, macOS hardlink aliases require enumeration to disambiguate names. Entries removed during directory snapshotting are skipped without failing the remaining listing or walk.
