...
Set `"slot_discovery": true` only on `openai-compat` providers backed by llama.cpp's `llama-server` or llama-swap: it makes go-llm read the server's `/props` `total_slots` so future slot-aware admission can size concurrency to the backend. Leave it off (the default) for backends without `/props` (vLLM, LM Studio) — an enabled backend that cannot answer `/props` is treated as having a single slot.
