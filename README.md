...
Optional opt-in: add `"slot_discovery": true` to an `openai-compat` provider backed by llama.cpp's `llama-server` or llama-swap (e.g. the `llamacpp` provider above) to make go-llm read the server's `/props` `total_slots` so future slot-aware admission can size concurrency to the backend. Leave it off (the default, and the shipped configuration) for backends without `/props` (vLLM, LM Studio) — an enabled backend that cannot answer `/props` is treated as having a single slot.
