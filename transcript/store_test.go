...
// TestOpen_WaitsForLockedWALFile pins the PRAGMA order: busy_timeout must
// precede journal_mode. On an existing WAL file, the journal_mode PRAGMA reads
// the database header and otherwise fails at once while another connection
// holds the database lock. Scheduling delay can only hide a regression: an
// opener that reaches the PRAGMA after the release passes under either order.
