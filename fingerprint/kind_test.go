...
func TestInferKindFromCapabilities(t *testing.T) {
	tests := []struct {
		...
		{"canonical chat capability", []string{"chat", "generate", "stream"}, ModelKindChat},
		{"canonical embed capability", []string{"embed"}, ModelKindEmbedding},
		{"canonical dual capability returns embedding kind", []string{"chat", "embed"}, ModelKindEmbedding},
