...
| Provider destinations | Config-driven Golem and MCP bootstrap require grants for remote model destinations; guarded transports check capabilities, origins and base paths and refuse redirects. Grants are revocable. | This is a provider boundary, not a process-wide firewall. It does not govern shell, external MCP, or consultant traffic. |
| MCP client | Workspace/alias catalog pins detect definition drift; tools require approval, have bounded execution/output, and produce foreign observations. | Pins do not attest endpoint/process identity. All admitted catalog tools are mounted. Stdio inherits the parent environment; HTTP is outside provider admission. |
...
| Provenance and audit | Observation fences, project-context trust, signed mutation receipts, signed memory records and offline integrity checks. | A signature is not authorization. Persistent origin/exposure continuity and complete policy-decision telemetry remain planned. |
...
|---|---|---|---|
| [#575](https://github.com/kstruzzieri/go-llm/issues/575) — default deterministic tool guards | P1 | 2–3 | Install argument invariants and egress labels by default. Specify compatibility for `-interceptors`, startup reporting and machine output; preserve the existing canary behavior. Labels remain advisory. |
...
| [#578](https://github.com/kstruzzieri/go-llm/issues/578) — MCP connection and environment boundaries | P1 | 5 | Explicit stdio environment allowlists and HTTP endpoint binding/redirect refusal for handshake, discovery, calls and cleanup. Full subprocess confinement is separate. Catalog pin identity remains workspace/alias/catalog unless explicitly amended. |
...
Backlog consolidation closes [#425](https://github.com/kstruzzieri/go-llm/issues/425) in favor of shipped #448 plus remaining caller-policy/ceiling decisions in #449 and scoped retrieval in #554. [#426](https://github.com/kstruzzieri/go-llm/issues/426) and [#427](https://github.com/kstruzzieri/go-llm/issues/427) transfer their complete remaining acceptance to #452, reusing #451. These closures transfer ownership; they do not claim all original work has shipped.
...
1. Start the bounded operational work: [#575](https://github.com/kstruzzieri/go-llm/issues/575), [#577](https://github.com/kstruzzieri/go-llm/issues/577), [#578](https://github.com/kstruzzieri/go-llm/issues/578), and [#579](https://github.com/kstruzzieri/go-llm/issues/579). Coordinate [#576](https://github.com/kstruzzieri/go-llm/issues/576) with #387's contract/reporting and verification approval identity; preserve existing #449/#552/#433 hardening scope.
2. Narrow grants with #420 and complete the individually sized observability issues. Develop [#580](https://github.com/kstruzzieri/go-llm/issues/580) against its own stdio launch seam, coordinated with [#578](https://github.com/kstruzzieri/go-llm/issues/578) and #406.
...
Points are provisional relative complexity, not engineer-day conversions. There is no inferred velocity, delivery date or fixed aggregate commitment. The existing [v0.4.0 scope](../README.md#roadmap) remains unchanged; these new issues are not all assigned to the next release. Milestone placement requires a separate capacity decision.
