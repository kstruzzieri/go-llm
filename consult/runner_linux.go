...
func groupExited(pgid int) bool {
	...
	return err == nil && zombieGroup("/proc", pgid)
}

// Linux keeps an unreaped zombie's process group visible to kill(0). Require
// positive evidence of a zombie-only group; unreadable or malformed proc data
// cannot establish cleanup. procRoot is supplied so failures can be tested.
func zombieGroup(procRoot string, pgid int) bool {
